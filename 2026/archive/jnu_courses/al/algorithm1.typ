...
#show: dvdtyp.with(
  ...
  subtitle: [note from book],
  author: "lencelg from Arcadia Bay",
)

#set text(font: "Source Han Sans")
#show raw: set text(font: ("Hack Nerd Font", "Source Han Sans"))
...
$ f(a, i) = a[0] "当" i = 1 "时" $
$ f(a, i) = max { f(a, i-1), a[i-1] } "当" i > 1 "时" $
...
有一个不带头结点的单链表 $L$，设计一个算法释放其中的所有结点。

解 设 $L = {a_1, a_2, ..., a_n}$，$f(L)$ 的功能是释放 $a_1 ~ a_n$ 的所有结点，则 $f(L -> "next")$ 的功能是释放 $a_2 ~ a_n$ 的所有结点，前者是“大问题”，后者是“小问题”。假设 $f(L -> "next")$ 已实现，则 $f(L)$ 就可以通过先调用 $f(L -> "next")$ 然后释放 $L$ 所指的结点来求解，如图 2.10 所示。对应的递归模型如下：
...
有一个不带头结点的单链表 $L$，设计一个算法删除其中所有结点值为 $x$ 的结点。

解: 设 $f(L, x)$ 的功能是删除以 $L$ 为首结点的单链表中所有结点值为 $x$ 的结点，是“大问题”，而 $f(L -> "next", x)$ 的功能是删除以 $L -> "next"$ 为首结点的单链表中所有结点值为 $x$ 的结点，是“小问题”。对应的递归模型如下：

$ f(L, x) = "不做任何事情" "当" L = "NULL" "时" $
$ f(L, x) = "删除 L 结点；L 指向原后继结点；" f(L, x) "当" L -> "data" = x "时" $
...
#image("img/2.11.png", width: 110%, height: 90%)
#image("img/2.22.png", width: 110%, height: 90%)

#pagebreak()

2.2.3 增加经验值

#image("img/2.3.png", width: 110%, height: 90%)
#image("img/bubblesort.png", width: 110%)

n 皇后

#image("img/n_queen.png", width: 80%)

分析时间复杂度

#image("img/time.png", width: 100%)
...
int Partition(int a[], int s, int t)    //划分算法
{
    ...
    while (i != j)                      //从序列两端交替向中间扫描，直到 i == j 为止
    {
        while (j > i && a[j] >= tmp)    //从右向左扫描，找第 1 个关键字小于 tmp 的 a[j]
            j--;
        a[i] = a[j];                    //将 a[j] 前移到 a[j] 的位置
        while (i < j && a[i] <= tmp)    //从左向右扫描，找第 1 个关键字大于 tmp 的 a[i]
            i++;
        a[j] = a[i];                    //将 a[i] 后移到 a[i] 的位置
    }
    a[i] = tmp;
    return i;
}
...
void MergePass(int a[], int length, int n)              // 一趟二路归并排序
{
    ...
    for (i = 0; i + 2 * length - 1 < n; i = i + 2 * length) // 归并 length 长的两个相邻子表
        Merge(a, i, i + length - 1, i + 2 * length - 1); // 余下两个子表，后者的长度小于 length
...
int BinSearch(int a[], int low, int high, int k)    // 折半查找算法
{
    ...
    if (low <= high)                // 当前区间存在元素时
    {
        ...
        if (a[mid] == k)            // 找到后返回其物理下标 mid
            return mid;
        else if (a[mid] > k)             // 当 a[mid] > k 时在 a[low..mid-1] 中递归查找
...
#figure(
  table(
    ...
    align: (left, center, center),    // 第一列左对齐，后两列居中
    [*对比维度*], [*子集树*], [*排列树*],
...
== 简单装载问题
有 $n$ 个集装箱要装上一艘载重量为 $W$ 的轮船，其中集装箱 $i (1 <= i <= n)$ 的重量为 $w_i$。不考虑集装箱的体积限制，现在要从这些集装箱中选出若干装上轮船，使它们的重量之和*恰好等于* $W$，同时要求选取的集装箱个数*尽可能少*。
...
给定有 $n$ 个不同正整数的集合 $w = (w_1, w_2, ..., w_n)$ 和一个正数 $W$，要求找出 $w$ 的子集 $s$，使该子集中所有元素的和为 $W$。例如，当 $n=4$ 时，$w = (11, 13, 24, 7)$，$W=31$，则满足要求的子集为 $(11, 13, 7)$ 和 $(24, 7)$。
...
对于本问题，所有活动按结束时间递增排序，需要证明：若 $X$ 是活动安排问题 $A$ 的最优解，且 $X = X' union {1}$，则 $X'$ 是 $A' = {i in A | b_i >= e_1}$ 的活动安排问题的最优解。

==== 贪心选择性质

- 设原问题最优解为 $X$，其中第一个被选中的活动为 $k$（$k != 1$）。
- 由于活动 1 的结束时间最早（$e_1 <= e_k$），且 $k$ 与 $X$ 中其他活动兼容，用活动 1 替换活动 $k$ 后，得到的新解 $Y'$ 仍然兼容。
...
- 若 $X$ 是原问题的最优解，则 $X' = X - {1}$ 必为子问题 $A' = {i in A | b_i >= e_1}$ 的最优解。
...
== 求解最优装载问题
*【问题描述】* 有 $n$ 个集装箱要装上一艘载重量为 $W$ 的轮船，其中集装箱 $i (1 <= i <= n)$ 的重量为 $w_i$。不考虑集装箱的体积限制，现要选出尽可能多的集装箱装上轮船，使它们的重量之和不超过 $W$。
...
- 设 $x$ 和 $y$ 是所有字符中*频率最低*的两个字符，即 $"freq"(x) <= "freq"(a)$ 且 $"freq"(y) <= "freq"(b)$。
- 现在将 $x$ 与 $a$ 交换位置，将 $y$ 与 $b$ 交换位置，得到新树 $T'$。
- 比较 $T$ 和 $T'$ 的代价（加权路径长度）：
  - 由于 $x$ 和 $y$ 的频率不高于 $a$ 和 $b$，将它们放到更深的深度，并不会增加总代价（实际上通常会减少或持平）。
- 因此，$T'$ 的代价 <= $T$ 的代价，由于 $T$ 已经是最优的，所以 $T'$ 也是最优的。
...
令给定的字符序列 $X = (x_0, x_1, ..., x_{m-1})$，序列 $Y = (y_0, y_1, ..., y_{k-1})$ 是 $X$ 的子序列，若存在 $X$ 的一个严格递增下标序列 $(i_0, i_1, ..., i_{k-1})$，使得对所有的 $j=0,1,...,k-1$ 有 $x_{i_j}=y_j$。例如，$X=(a,b,c,b,d,a,b)$，$Y=(b,c,d,b)$ 是 $X$ 的一个子序列。
...
#image("img/rev.png", width: 100%)

== 0/1 背包问题

递推关系：
- 边界：$"dp"[i][0] = 0$（背包容量为0），$"dp"[0][r] = 0$（没有物品）
- 当 $r < w[i]$ 时：$"dp"[i][r] = "dp"[i-1][r]$
- 否则：$ "dp"[i][r] = max("dp"[i-1][r], "dp"[i-1][r-w[i]] + v[i])$
...
两者的包含关系: $P subset.eq "NP"$

解释：如果你能 *“快速求出”* 一个问题的解（P），那你当然也能在 *“快速验证”* 这个解是否正确（NP）。所以所有 P 类问题都属于 NP 类问题。
