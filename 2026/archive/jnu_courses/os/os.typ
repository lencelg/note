...
== 基本概念
操作系统的组成：
- 硬件：中央处理机（运算器和控制器）、储存器、输入设备和输出设备等。（物质基础和工作环境）
- 软件：系统软件（操作系统、多种语言处理程序【汇编和编译程序】、连接转配程序、系统实用程序）和应用软件（应用编制的程序）。
+ 裸机：没有任何软件支持的计算机。
+ 虚拟机/拓展机：操作系统+裸机。
...
- 作为用户和计算机硬件之间的接口
  - 命令接口
  - 程序接口（=系统调用=广义指令）用户间接调用, 由一组系统调用组成
  - GUI
...
2. 晶体管时代，批处理系统
3. 集成电路时代，多道程序设计
4. 大规模和超大规模集成电路时代，分时系统

== 中断
中断的本质：#text(fill: red)[发生中断就意味着需要操作系统介入，展开管理工作]

特点： 
+ #text(fill: red)[有了中断才能实现程序并发执行]
+ #text(fill: red)[用户态到核心态的转换是通过中断实现的，并且是唯一途径]
+ 中断发生时，cpu立即进入核心态
+ 不同的中断信号会进行不同的处理。

下面是#text(fill: blue)[中断的分类]

#table(
    columns: (2.1fr, 1fr),
    [#align(center)[*内中断*(cpu内部，与当前执行的指令有关)]], [#align(center)[*外中断*(cpu外部，与当前执行的指令无关)]],
    [
      #align(center)[分类1]
      #table(
        columns: (1fr, 1fr),
    ...
    [
      ...
      #table(
        ...
        stroke: 0.5pt + green,
        [ 1. 故障(fault)：由错误条件引起的],
        [ 2. 终止(abort)：不可恢复的致命错误，操作系统介入也无法恢复],
        [ 3. 陷阱(trap)：有意而为之的异常]
      )
    ], [#align(center)[外设请求]],
)

系统调用运行在#text(fill: red)[核心态]

== 作业
一个作业是在一次应用业务的处理过程中，从输入开始到输出结束，用户要求计算机#text(fill: red)[所做的有关该次业务处理的全部工作]

$ "作业" eq "程序" plus "数据" plus "作业说明书" $

作业的概念一般用于早起的批处理系统和现在的大型机、巨型机中，#text(fill: red)[现在一般不用了]

#text(fill: blue)[一个作业可包含多个程序和多个数据集，但必须至少包含一个程序]
...
内核级线程
- 在核心态下完成，由操作系统内核完成
- #text(fill: red)[只有内核级线程才是处理机分配的单位]

#figure(
  table(
    columns: (1fr, 2fr, 1.1fr, 1fr, 1.5fr),
    align: center,
    // 表头
   [], [要做什么], [调度发生在...], [发生频率], [对进程状态的影响],
    // 第一行数据
   [高级调度#linebreak()（作业调度）],  [按照某种规则，从后备队列中选择合适的作业将其调入内存，并为其创建进程],
    [外存→内存#linebreak()（面向作业）],
    [最低],
    [无→创建态→就绪态],
    // 第二行数据
    [中级调度#linebreak() （内存调度）],
    [按照某种规则，从挂起队列中选择合适的进程将其数据调回内存],
    [外存→内存#linebreak()（面向进程）],
    [中等],
    [挂起态→就绪态#linebreak()（阻塞挂起→阻塞态）],
    // 第三行数据
    [低级调度#linebreak()（进程调度）],
...
#text(fill: blue)[普通正常的临界区是可以被调度的，但是内核程序的临界区不能进行进程调度。]

#pagebreak()
= PV 同步

前V后P（前面执行的代码执行后进行V操作，后面的代码执行之前进行P操作）
...
#figure(
  table(
    ...
    [解法一：加互斥锁], [限制最多 4 人进入取筷区], [破坏 *“循环等待”*], [较高],
    [解法二：奇偶反序], [改变拿取方向，避免环形依赖], [破坏 *“循环等待”*], [最高],
    [解法三：原子拿取], [两根筷子一起拿], [破坏 *“持有并等待”*], [较低]
  ),
  caption: [哲学家就餐问题解法对比]
)
...
*题目*：有一阅览室，共有 100 个座位，为了很好地利用它，读者进入时必须先在登记表上进行登记。该表表目设有座位号和读者姓名，离开时再将其登记删除。试问：
...
#figure(
  table(
    columns: (auto, 1.5fr, 1.5fr, 1fr),
    align: (left, left, left, center),
    [*数据结构*], [*符号*], [*含义*], [*维度*],
    [可利用资源向量], [`Available`], [系统当前*还剩*多少各类资源], [长度为 `m` 的一维数组],
    [最大需求矩阵], [`Max`], [每个进程*总共最多*需要多少各类资源], [`n times m` 矩阵],
    [分配矩阵], [`Allocation`], [每个进程*当前已经占*了多少各类资源], [`n times m` 矩阵],
    [需求矩阵], [`Need`], [每个进程*还缺*多少各类资源], [`n times m` 矩阵]
  ),
  caption: [银行家算法数据结构]
)
...
建议跟着图片练习（后面 LLM 的生成习题当作是额外的理解就好了）
...
源程序经过编译，得到的目标程序以“0”作为参考地址模块，然后多个目标模块由链接程序链接成一个具有统一地址的装配模块，最后再装入到内存。把目标模块中的地址称为#text(fill: blue)[相对地址（逻辑地址）], 装配模块虽然具有统一的地址空间，但它仍是以“0”作为参考地址，即是浮动的。

要把它#text(fill: blue)[装入内存执行]，就要确定装入内存的实际物理地址，并修改程序中与地址有关的代码，这一过程叫做#text(fill: blue)[地址重定位]

按照重定位的时机，可分为#text(fill: blue)[静态重定位]和#text(fill: blue)[动态重定位]
- 静态地址重定位
  - 在程序执行之前进行重定位，根据装配模块将要装入的内存起始位置，直接修改装配模块中的有关使用地址的指令。#text(fill: blue)[相当于直接平移]
  - 具有无需硬件支持的优点, 但是程序重定位后就不能在内存中搬动了, 并且需要连续的存储空间
- 动态地址重定位
  - 在程序执行过程中进行地址重定位, 在CPU每次访问内存单元前才进行地址变换。
  - 但是需要定位寄存器的支持
