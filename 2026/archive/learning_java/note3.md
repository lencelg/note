---
title: java cross-platform breif introduction
date: jvm note
---
> [!NOTE]
>
> this post is generated with the help of llm, just for a brief unstanding of java cross-platform implementation
...
用 `javap -v Hello.class` 查看反编译结果，可以看到：
...
## 设计原因
Java 字节码用一套**固定的、紧凑的、设计时考虑过的中间指令**来编码, 最终由 JVM 把它翻译成目标平台的机器码，实现跨平台。
...
| **方法区**| 线程共享 | 类的结构信息、常量池、静态变量、JIT 编译后的代码 | 部分回收 |
...
**混合模式**：JVM 默认启动时先用解释器快速启动，后台 JIT 线程异步编译热点方法。编译完成后，下次调用自动切换到编译后的版本。这就是 **分层编译**（Tiered Compilation）。
...
## 本地方法接口（JNI）：调用系统底层
当 Java 需要调用操作系统功能（比如读写文件、发送网络包）或访问特定硬件时，会通过 **Java Native Interface (JNI)** 调用用 C/C++ 编写的本地库。这些本地方法的执行进入**本地方法栈**，由操作系统直接处理。
